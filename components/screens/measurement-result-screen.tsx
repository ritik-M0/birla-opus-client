...
import { Pencil, Check, X, LayoutPanelTop, AlertCircle } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { AppHeader } from "@/components/app-header"
import { WallDiagram } from "@/components/wall-diagram"
import { CeilingDiagram } from "@/components/ceiling-diagram"
...
export function MeasurementResultScreen({ surface, onBack, onSave }: MeasurementResultScreenProps) {
  const [measurements, setMeasurements] = useState<SurfaceMeasurement>(
    surface.measurements || defaultMeasurements
  )
  const [editingField, setEditingField] = useState<keyof SurfaceMeasurement | null>(null)
  const [editValue, setEditValue] = useState("")
  const [showDiagram, setShowDiagram] = useState(false)
  const [editingObjectId, setEditingObjectId] = useState<string | null>(null)
  const [editingObjectField, setEditingObjectField] = useState<"name" | "area" | null>(null)
  ...
  const handleToggleBlockedObject = (objectId: string) => {
    setMeasurements((prev) => ({
      ...
      blockedObjects: prev.blockedObjects?.map((obj) =>
        obj.id === objectId ? { ...obj, addedToPaintable: !obj.addedToPaintable } : obj
      ),
    }))
  }

  const handleEditBlockedObject = (objectId: string, field: "name" | "area") => {
        ...
        blockedObjects: prev.blockedObjects?.map((obj) =>
          obj.id === editingObjectId
            ? {
                ...obj,
                [editingObjectField]:
                  editingObjectField === "area" ? parseFloat(editingObjectValue) || 0 : editingObjectValue,
              }
            : obj
        ),
      }))
  ...
  const fields: { key: keyof SurfaceMeasurement; label: string; dot: string }[] = [
    {
      key: "totalArea",
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex-1 px-5 pt-5 overflow-y-auto pb-2">
        <div className="flex items-start justify-between mb-5">
          <div>
            <h2 className="text-xl font-bold text-foreground tracking-tight">Measurement Results</h2>
            <p className="text-sm text-muted-foreground mt-1">Review and adjust values if needed</p>
          </div>
          <button
        ...
        {showDiagram && (
          <div className="mb-5">
            {surface.type === "ceiling" ? (
              <CeilingDiagram measurements={measurements} surfaceName={surface.name} />
            ) : (
              <WallDiagram measurements={measurements} surfaceName={surface.name} />
            )}
          </div>
        )}
        ...
        <div className="space-y-3">
          {fields.map(({ key, label, dot }) => (
            <div key={key} className="p-4 bg-card rounded-2xl border border-border">
              ...
              {key === "totalArea" && (
                <div className="flex items-center gap-1.5 mt-2 pt-2 border-t border-border">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Please review total {surface.type === "ceiling" ? "ceiling" : "wall"} area before proceeding
                  </p>
                </div>
              )}

              {/* Blocked area object-wise breakdown */}
              {key === "blockedArea" && measurements.blockedObjects && measurements.blockedObjects.length > 0 && (
                <div className="mt-3 pt-3 border-t border-border">
                  <p className="text-xs font-semibold text-foreground mb-2.5">Blocked Area Detected</p>
                  <div className="space-y-2">
                    {measurements.blockedObjects.map((obj) => (
                      <div key={obj.id} className="p-3 bg-muted rounded-lg">
                        <div className="flex items-center justify-between mb-2.5">
                          <div className="flex-1 min-w-0">
                            {editingObjectId === obj.id && editingObjectField === "name" ? (
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  onClick={() => handleEditBlockedObject(obj.id, "name")}
                            ...
                            {editingObjectId === obj.id && editingObjectField === "area" ? (
                              ...
                              <div className="flex items-center gap-1 text-xs text-muted-foreground mt-0.5">
                                <span>
                                  {obj.area} <span className="text-muted-foreground">sq.ft</span>
                                </span>
                                <button
                                  onClick={() => handleEditBlockedObject(obj.id, "area")}
                                  className="rounded-lg hover:bg-background p-1"
                                >
                                  <Pencil className="w-3 h-3 text-muted-foreground" />
                                </button>
