...
import { CheckCircle2, Camera, SquareDashedBottom, Lock, Plus, Trash2, Pencil, Check, X } from "lucide-react"
...
export function WallSelectionScreen({ room, onBack, onSelectSurface, onRoomChange, onProceed }: WallSelectionScreenProps) {
  ...
  const isCeilingUnlocked = (): boolean => walls.every((w) => w.status === "completed")

  const allComplete =
    walls.every((w) => w.status === "completed") && (!ceiling || ceiling.status === "completed")
  ...
  const commitRename = () => {
    ...
    if (ceiling && renamingId === ceiling.id) {
      ...
      update({ ...localRoom, walls: walls.map((w) => w.id === renamingId ? { ...w, name: trimmed } : w) })
    }
    setRenamingId(null)
  }
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex-1 px-5 pt-5 overflow-y-auto pb-6">
        ...
        <div className="flex items-center justify-between mb-3">
          <p className="text-[11px] font-semibold text-muted-foreground tracking-widest uppercase">Walls</p>
          <button
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          {walls.map((surface, index) => {
            ...
            const canRemove = surface.status === "not-started" && walls.length > 1
            return (
              <SurfaceCard
                key={`wall-${surface.id}`}
                surface={surface}
        ...
        {/* Ceiling section */}
        <div className="flex items-center justify-between mb-3">
          <p className="text-[11px] font-semibold text-muted-foreground tracking-widest uppercase">Ceiling</p>
          {!ceiling && (
            <button
            ...
            >
              <Plus className="w-3.5 h-3.5 text-navy" />
              <span className="text-xs font-semibold text-navy">Add Ceiling</span>
            </button>
          )}
        </div>
        ...
        {allComplete && (
          <div className="mt-5 p-4 bg-success/8 border border-success/20 rounded-2xl flex items-center gap-3 mb-5">
            ...
            <p className="text-sm font-semibold text-success">All surfaces measured</p>
          </div>
        )}
      ...
      >
        ...
        >
          {locked ? (
            ...
            <Camera className={`w-5 h-5 ${done ? "text-success" : "text-navy"}`} />
          )}
        </div>
        ...
        {isRenaming ? (
          <div className="flex items-center gap-1 w-full" onClick={(e) => e.stopPropagation()}>
            ...
            />
            <button onClick={onRenameCommit} className="shrink-0"><Check className="w-3 h-3 text-success" /></button>
            <button onClick={onRenameCancel} className="shrink-0"><X className="w-3 h-3 text-muted-foreground" /></button>
          </div>
        ) : (
          <p className={`text-sm font-semibold text-center ${locked ? "text-muted-foreground" : "text-foreground"}`}>
            {surface.name}
          </p>
        )}

        <p className={`text-xs mt-0.5 ${done ? "text-success" : "text-muted-foreground"}`}>
          {done ? `${surface.measurements?.paintableArea ?? 0} sq.ft` : locked ? "Locked" : "Not started"}
        </p>
      </button>
    </div>
  )
    ...
    >
      ...
      >
        {locked ? (
          ...
          <SquareDashedBottom className={`w-5 h-5 ${done ? "text-success" : "text-navy"}`} />
        )}
      </div>
      ...
      <div className="flex-1 text-left">
        {isRenaming ? (
          <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
            ...
            />
            <button onClick={(e) => { e.stopPropagation(); onRenameCommit() }}><Check className="w-3.5 h-3.5 text-success" /></button>
            <button onClick={(e) => { e.stopPropagation(); onRenameCancel() }}><X className="w-3.5 h-3.5 text-muted-foreground" /></button>
          </div>
        ) : (
          <p className={`text-sm font-semibold ${locked ? "text-muted-foreground" : "text-foreground"}`}>
            {ceiling.name}
          </p>
        )}
        <p className={`text-xs mt-0.5 ${done ? "text-success" : "text-muted-foreground"}`}>
